...
cd /opt
sudo git clone https://github.com/gongiskhan/airbyte-poc.git
cd airbyte-poc

# Copy our updated AirbyteClient for self-hosted
sudo tee airbyte-client.js > /dev/null << 'EOL'
/**
...
sudo sed -i 's|https://api.airbyte.com/v1|http://localhost:8000/api/v1|g' server.js

# Remove the client ID and secret requirements
sudo sed -i 's|const AIRBYTE_CLIENT_ID = process.env.AIRBYTE_CLIENT_ID || .*|const AIRBYTE_CLIENT_ID = null; // Not needed for self-hosted|g' server.js
sudo sed -i 's|const AIRBYTE_CLIENT_SECRET = process.env.AIRBYTE_CLIENT_SECRET || .*|const AIRBYTE_CLIENT_SECRET = null; // Not needed for self-hosted|g' server.js

# Update the AirbyteClient initialization
sudo sed -i 's|const airbyteClient = new AirbyteClient({|const airbyteClient = new AirbyteClient({\n  // Using self-hosted Airbyte with no auth\n  useBasicAuth: false,|g' server.js
...
echo "Setup complete!"
echo "Airbyte UI is available at: http://YOUR_VM_IP:8000"
echo "Airbyte POC application is available at: http://YOUR_VM_IP:3000"
echo ""
echo "To get your VM's external IP, run: gcloud compute instances describe airbyte-poc-vm --zone=us-central1-a --format='get(networkInterfaces[0].accessConfigs[0].natIP)'"
