...
# Clone or update the Airbyte POC repository
echo "Setting up Airbyte POC application..."
cd ~
if [ ! -d "airbyte-poc" ]; then
    echo "Cloning Airbyte POC repository..."
    git clone https://github.com/gongiskhan/airbyte-poc.git
    cd airbyte-poc
else
    echo "Airbyte POC repository already exists, pulling latest changes..."
    cd airbyte-poc
    git pull
fi
